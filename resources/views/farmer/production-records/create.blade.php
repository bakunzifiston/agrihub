<x-tenant-layout>
    ...
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6 max-w-2xl">
        <form method="POST" action="{{ route('farmer.production-records.store') }}">
            @csrf
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <x-text-input id="production_date" name="production_date" type="date" class="mt-1 block w-full" :value="old('production_date')" required />
                    <x-input-error class="mt-2" :messages="$errors->get('production_date')" />
                </div>
                <div>
                    <x-input-label for="quantity_produced" value="Quantity Produced *" />
                    <x-text-input id="quantity_produced" name="quantity_produced" type="number" step="0.01" min="0" class="mt-1 block w-full" :value="old('quantity_produced')" required />
                    <x-input-error class="mt-2" :messages="$errors->get('quantity_produced')" />
                </div>
                <div>
                    <x-input-label for="quantity_unit" value="Unit *" />
                    <x-text-input id="quantity_unit" name="quantity_unit" type="text" class="mt-1 block w-full" :value="old('quantity_unit')" placeholder="e.g. kg, tons" required />
                    <x-input-error class="mt-2" :messages="$errors->get('quantity_unit')" />
                </div>
                <div>
                    <x-input-label for="quality_grade" value="Quality Grade" />
                    <x-text-input id="quality_grade" name="quality_grade" type="text" class="mt-1 block w-full" :value="old('quality_grade')" />
                    <x-input-error class="mt-2" :messages="$errors->get('quality_grade')" />
                </div>
                <div>
                    <x-input-label for="losses_quantity" value="Losses" />
                    <x-text-input id="losses_quantity" name="losses_quantity" type="number" step="0.01" min="0" class="mt-1 block w-full" :value="old('losses_quantity', 0)" />
                    <x-input-error class="mt-2" :messages="$errors->get('losses_quantity')" />
                </div>
                <div class="md:col-span-2">
                    <x-input-label for="remarks" value="Remarks" />
                    <textarea id="remarks" name="remarks" rows="2" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary focus:ring-primary">{{ old('remarks') }}</textarea>
                    <x-input-error class="mt-2" :messages="$errors->get('remarks')" />
                </div>
            </div>
            <div class="mt-6 flex gap-4">
                <x-primary-button>Add Record</x-primary-button>
