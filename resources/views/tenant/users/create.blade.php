<x-tenant-layout>
    ...
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6 max-w-2xl">
        ...
        <form method="POST" action="{{ route(auth()->user()->tenant_type . '.users.store') }}">
            @csrf
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-500 mb-2">Select permissions to grant (role already includes base permissions)</p>
                    <div class="mt-2 space-y-2 max-h-48 overflow-y-auto border border-gray-200 rounded-md p-3 bg-gray-50">
                        @foreach ($permissions as $perm)
                            <label class="flex items-center gap-2 text-sm">
                                <input type="checkbox" name="permissions[]" value="{{ $perm->name }}" {{ in_array($perm->name, old('permissions', [])) ? 'checked' : '' }} class="rounded border-gray-300 text-primary focus:ring-primary">
