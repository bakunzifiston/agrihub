<x-tenant-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Cooperative Inventory</h2>
            <a href="{{ route('cooperative.inventory.create') }}" class="inline-flex items-center px-4 py-2 bg-primary text-white rounded-md hover:bg-primary-700 font-medium text-sm">Add Item</a>
        </div>
    </x-slot>
    ...
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
        @if ($inventory->isEmpty())
            ...
            <a href="{{ route('cooperative.inventory.create') }}" class="mt-4 inline-flex items-center px-4 py-2 bg-primary text-white rounded-md hover:bg-primary-700">Add Item</a>
        @else
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @foreach ($inventory as $item)
                            <tr>
                                ...
                                <td class="px-4 py-3 text-sm text-gray-600">{{ $item->last_updated?->format('M d, Y H:i') ?? $item->updated_at->format('M d, Y') }}</td>
                                <td class="px-4 py-3 text-right">
                                    <a href="{{ route('cooperative.inventory.edit', $item) }}" class="text-primary hover:text-primary-700 text-sm font-medium mr-3">Edit</a>
                                    <form method="POST" action="{{ route('cooperative.inventory.destroy', $item) }}" class="inline" onsubmit="return confirm('Remove this item?');">
                                        @csrf
                                        @method('DELETE')
