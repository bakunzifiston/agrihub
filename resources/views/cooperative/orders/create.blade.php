<x-tenant-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Add Order</h2>
    ...
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6 max-w-3xl">
        <form method="POST" action="{{ route('cooperative.orders.store') }}">
            @csrf
            @include('cooperative.orders._form', ['order' => null, 'clients' => $clients ?? collect(), 'inventoryItems' => $inventoryItems ?? collect()])
            <div class="mt-6 flex gap-4">
                <x-primary-button>Add Order</x-primary-button>
