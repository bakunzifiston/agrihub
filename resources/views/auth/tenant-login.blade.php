<x-guest-layout>
    ...
    <h2 class="text-xl font-semibold text-stone-800 mb-1">{{ $tenantLabel }} Portal</h2>
    <p class="text-sm text-stone-500 mb-6">Sign in to your account</p>

    <x-auth-session-status class="mb-4" :status="session('status')" />

    <form method="POST" action="{{ route($tenantType . '.login.post') }}">
        @csrf
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mt-6 gap-4">
            <a class="underline text-sm text-stone-600 hover:text-stone-900" href="{{ route($tenantType . '.register') }}">
