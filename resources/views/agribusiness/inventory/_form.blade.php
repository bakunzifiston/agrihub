...
<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
    ...
    <div>
        ...
        <x-text-input id="category" name="category" type="text" class="mt-1 block w-full" :value="old('category', $inventory?->category)" placeholder="e.g. Processed, Raw" />
        <x-input-error class="mt-2" :messages="$errors->get('category')" />
    </div>
    <div>
        <x-input-label for="quantity_in_stock" value="Quantity in Stock *" />
        <x-text-input id="quantity_in_stock" name="quantity_in_stock" type="number" step="0.01" min="0" class="mt-1 block w-full" :value="old('quantity_in_stock', $inventory?->quantity_in_stock ?? 0)" required />
        <x-input-error class="mt-2" :messages="$errors->get('quantity_in_stock')" />
    </div>
    <div>
        <x-input-label for="unit" value="Unit *" />
        <x-text-input id="unit" name="unit" type="text" class="mt-1 block w-full" :value="old('unit', $inventory?->unit)" placeholder="e.g. kg, bags" required />
    ...
    <div>
        ...
        <x-text-input id="batch_number" name="batch_number" type="text" class="mt-1 block w-full" :value="old('batch_number', $inventory?->batch_number)" />
        <x-input-error class="mt-2" :messages="$errors->get('batch_number')" />
    </div>
    <div>
        <x-input-label for="expiry_date" value="Expiry Date" />
        <x-text-input id="expiry_date" name="expiry_date" type="date" class="mt-1 block w-full" :value="old('expiry_date', $inventory?->expiry_date?->format('Y-m-d'))" />
